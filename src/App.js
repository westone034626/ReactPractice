...
function App() {
  const [focusedGraphicItemIndex, setFocusedGraphicItemIndex] = useState(0);
  const targetIORef = useRef(0);
  ...
  useEffect(() => {
    const io = new IntersectionObserver((entries, observer) => {
      targetIORef.current = _.chain(stepItemListRef.current.children)
  ...
  useEffect(() => {
    window.addEventListener('scroll', () => {
      _.chain(stepItemListRef.current.children)
        .map((stepItem) => stepItem)
        .slice(
          targetIORef.current ? targetIORef.current - 1 : 0,
          targetIORef.current + 2
        )
        .forEach((stepItem) => {
          if (
            ...
            const idx = _.chain(stepItemListRef.current.children)
              .map((stepItem) => stepItem)
              .value()
              .indexOf(stepItem);
            setFocusedGraphicItemIndex(idx);
          }
        })
        .value();
    });
