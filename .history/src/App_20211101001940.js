...
function App() {
  ...
  useEffect(() => {
    window.addEventListener('scroll', () => {
      // 10개의 stepItem을 스크롤마다 검사하며 window의 특정 영역에 있는지 검사하고 만약 특정 영역 안에 있다면
      // log로 출력
      stepItemListRef.current &&
        console.log(typeof stepItemListRef.current.children);
      // stepItemListRef.current.children.forEach((stepItem, idx, array) => {
      //   if (
      //     stepItem.getBoundingClientRect().top > window.innerHeight * 0.1 &&
      //     stepItem.getBoundingClientRect().top < window.innerHeight * 0.8
      //   )
      //     console.log('array.indexOf(stepItem): ', array.indexOf(stepItem));
      // });
    });
  }, []);
  return (
      ...
      <div className={styles.scrollContent}>
        <div className={styles.scrollGraphic}>
          {graphicItems.map((graphicItem, idx) => (
            <div key={idx} className={styles.graphicItem}>
              <img
